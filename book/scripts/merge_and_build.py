...
def merge_markdown_files(temp_dir):
    ...
    if ordered_files is not None:
        ...
        # 与 run_state.py 保持一致：空源 chunk（convert 分块产生的尾部空块）允许空输出
        for fp in output_files:
            src = os.path.join(temp_dir, os.path.basename(fp)[len("output_"):])
            source_empty = os.path.exists(src) and os.path.getsize(src) == 0
            if os.path.getsize(fp) == 0:
                if source_empty:
                    print(f"SKIP: {os.path.basename(fp)} empty (source chunk also empty) — skipping")
                    continue
            ...
            if not text.strip():
                if source_empty:
                    print(f"SKIP: {os.path.basename(fp)} blank (source chunk also empty) — skipping")
                    continue
