...
def build_chunk_record(temp_dir, chunk_id):
    ...
    source_empty = os.path.getsize(source_path) == 0
    # 空源 chunk（转换产物的尾部空块，如 chunk0200）无内容可译，允许空输出；
    # 仅对非空源块强制校验输出非空/可读/非纯空白。
    if not source_empty:
...
def plan(temp_dir, retranslate_untracked=False):
    ...
    for entry in entries:
        ...
        source_empty = os.path.exists(source_path) and os.path.getsize(source_path) == 0
        if not os.path.exists(output_path):
            item["action"] = "translate"
            _reason(item, "missing_output")
        elif os.path.getsize(output_path) == 0 and not source_empty:
            item["action"] = "translate"
            _reason(item, "empty_output")
        elif not source_empty:
            output_text = read_output_text(output_path)
            if output_text is None:
                item["action"] = "translate"
